...
After the release is approved, merge it into both main and develop.

Then create a version tag on main.
...
main → to fix the production problem
develop → so the same fix is included in future releases
Branch Summary
Branch	Purpose
...
Also merge the release branch back into develop.
...
Also merge the hotfix into develop.
...
Delete feature and bugfix branches after they are merged.
Hotfixes must be merged into both main and develop.
Release branches are for testing and final preparation, not for adding new features.
